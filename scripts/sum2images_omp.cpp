...
cv::Mat sumImages(const cv::Mat& img1, const cv::Mat& img2) {
    ...
    // Use OpenMP to parallelize the image addition operation
    #pragma omp parallel for
    for (int y = 0; y < result.rows; ++y) {
        for (int x = 0; x < result.cols; ++x) {
            // Add pixels from both resized images
            for (int c = 0; c < img1.channels(); ++c) {
                result.at<cv::Vec3b>(y, x)[c] = cv::saturate_cast<uchar>(
                    img1_resized.at<cv::Vec3b>(y, x)[c] + img2_resized.at<cv::Vec3b>(y, x)[c]);
            }
        }
    }
